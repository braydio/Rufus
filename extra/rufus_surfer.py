...
async def query_chat_completion(messages):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(AI_API_URL, json=payload, timeout=560) as resp:
                if resp.status != 200:
...
async def on_message(message):
    ...
    # 👇 Append full message (raw + embed) to session tracker
    rsa_tracker.append_message(user_id, full_text)
    # ✅ Append every message to session log
    rsa_tracker.append_message(user_id, message.content)
